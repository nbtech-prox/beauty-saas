...
describe("TenantSchema", () => {
  ...
  it("rejects invalid slug (uppercase)", () => {
    expect(() =>
      TenantSchema.parse({ ...validTenant, slug: "Bad-Slug" }),
    ).toThrow();
  });

  it("rejects invalid email", () => {
    expect(() =>
      TenantSchema.parse({ ...validTenant, ownerEmail: "not-email" }),
    ).toThrow();
  });
...
describe("TenantCreateInputSchema", () => {
  ...
  it("rejects the legacy planId field when planCode is missing", () => {
    expect(() =>
